...
def backDeformVerts(verts, RMats, trans, J, weights, kintree_table, parent):
    numJoints = J.shape[0]
    ...
    # transformation of each vertex
    T = np.tensordot(weights[:numVerts, :], G, axes=[[1], [0]])
    for iV in range(T.shape[0]):
        if np.isfinite(np.linalg.cond(T[iV, :, :])):
            T[iV, :, :] = np.linalg.inv(T[iV, :, :] )

    rest_shape_h = np.hstack((verts - trans.reshape([1, 3]), np.ones([verts.shape[0], 1])))
    v = np.matmul(T, rest_shape_h.reshape([-1, 4, 1])).reshape([-1, 4])[:, :3]
    verts = v
